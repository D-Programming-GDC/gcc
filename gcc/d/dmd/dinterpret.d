...
public:
    ...
    static Expression interpretVectorToArray(UnionExp* pue, VectorExp e)
    {
        if (e.e1.op == TOK.arrayLiteral)
            return cast(ArrayLiteralExp)e.e1;
        if (e.e1.op == TOK.int64 || e.e1.op == TOK.float64)
        {
            ...
            {
                TypeVector tv = cast(TypeVector)e.type;
                if (tv.basetype.ty == Tsarray)
                    type = cast(TypeSArray)tv.basetype;
            }
            else if (e.type.ty == Tsarray)
                type = cast(TypeSArray)e.type;
            assert(type);
    ...
    static bool resolveIndexing(IndexExp e, InterState* istate, Expression* pagg, uinteger_t* pidx, bool modify)
    {
        ...
        {
            UnionExp ue = void;
            e1 = interpretVectorToArray(&ue, cast(VectorExp)e1);
    ...
    override void visit(SliceExp e)
    {
        ...
        if (e1.op == TOK.vector)
        {
            e1 = interpretVectorToArray(pue, cast(VectorExp)e1);
    ...
    override void visit(PtrExp e)
    {
        ...
        if (e.e1.op == TOK.symbolOffset && (cast(SymOffExp)e.e1).offset == 0 && (cast(SymOffExp)e.e1).var.isVarDeclaration() && isFloatIntPaint(e.type, (cast(SymOffExp)e.e1).var.type))
        {
            ...
            result = paintFloatInt(pue, getVarExp(e.loc, istate, (cast(SymOffExp)e.e1).var, ctfeNeedRvalue), e.type);
            return;
        }
...
private Expression scrubReturnValue(const ref Loc loc, Expression e)
{
    ...
    else if (e.op == TOK.vector)
    {
        auto ve = cast(VectorExp)e;
        ve.ownedByCtfe = OwnedBy.code;
        if (ve.e1.op == TOK.arrayLiteral)
        {
            auto ale = cast(ArrayLiteralExp)ve.e1;
            ale.ownedByCtfe = OwnedBy.code;
            if (auto ex = scrubArray(loc, ale.elements))
                return ex;
        }
    }
    return e;
...
private Expression scrubCacheValue(const ref Loc loc, Expression e)
{
    ...
    else if (e.op == TOK.vector)
    {
        ...
        if (ve.e1.op == TOK.arrayLiteral)
        {
            ...
            if (auto ex = scrubArrayCache(loc, ale.elements))
                return ex;
        }
    }
    return e;
